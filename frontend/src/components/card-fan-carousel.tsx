...
const ARROW_CLASSES =
  'relative flex items-center justify-center rounded-full border-[1.5px] border-white/15 bg-ink-900/70 backdrop-blur-md text-white/60 cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.4)] hover:border-gold-400/60 hover:text-gold-300 active:opacity-70 transition-colors duration-300'
          ...
          {cards.map((card, index) => {
            const image = (
              <div className="relative h-full w-full overflow-hidden">
                ...
                />
                {card.subtitle && (
                  <span className="absolute left-3 top-3 z-30 rounded-full bg-ink-950/80 px-2.5 py-1 text-[11px] font-medium text-gold-300">
                    {card.subtitle}
                  </span>
                )}
                <div className="pointer-events-none absolute inset-0 z-20 bg-gradient-to-t from-ink-950/85 via-ink-950/5 to-transparent" />
                {card.title && (
                  <p className="absolute inset-x-0 bottom-0 z-30 p-4 font-display text-base font-semibold leading-tight text-white">
                    {card.title}
                  </p>
                )}
              </div>
            ...
            if (card.linkUrl) {
              return card.linkUrl.startsWith('http') ? (
                <a
                  key={index}
                  href={card.linkUrl}
                  target="_blank"
